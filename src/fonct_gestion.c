...
int disasmcmd(interpreteur inter, mem vmem, reg* tab_reg)    
{
    ...
    if (strcmp(token2,":")==0 && get_type(token1)==HEXA && get_type(token3)==HEXA) /* range avec deux adresses */
    {
        ...
        return _disasm_range_hexacmd(token1, token3, vmem, tab_reg);
    }
    ...
    else if (strcmp(token2,"+")==0 && get_type(token1)==HEXA && get_type(token3)==INTEGER) /* range avec offset */
    {
        ...
        INFO_MSG("La commande disasm %s + %s est exécutée", token1, token3);
        return _disasm_range_offsetcmd(token1, offsetValue, vmem, tab_reg);
    }
...
int dispcmd(interpreteur inter, reg* tab_reg, mem vmem)
{
	...
	if (get_type(token2)!=HEXA) 
	    ...
	    {if (strcmp(token3, "+")!=0) {WARNING_MSG("Expecting a range of address, but there isn't ':' after %s", token2); return 1;}}
    ...
    else if (token3!=NULL || token4!=NULL) // s'il ne doit pas y avoir de plage de donnée, on vérifie qu'il n'y a rien d'autre
    {
    ...
    {   
        ...
        {       
            ...
            return _disp_mem_plagescmd(token2, token4, vmem); // on exécute la commande avec les deux adresses indiquées 
        }
        ...
        {       
            ...
            INFO_MSG("La commande disp mem %s + %s est exécutée", token2, token4);
            return _disp_mem_offsetcmd(token2, offsetValue, vmem); // on exécute la commande avec les deux adresses indiquées
        }
