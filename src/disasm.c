...
int _disasm_range_hexacmd(char* addr1, char* addr2, mem vmem, reg* tab_reg)
{
	...
	if (k!=0 && k!=CMD_BREAK_RETURN_VALUE && k!=CMD_EXIT_RETURN_VALUE) // erreur dans realise_instr
	{
	    WARNING_MSG("La fonction realise_instr a retourné une erreur, arrêt du désassemblage");
            return 1;
	}

	if (k==CMD_EXIT_RETURN_VALUE) // EXIT à cause de SYSCALL
	{
            return k;
	}

	if(k==CMD_BREAK_RETURN_VALUE)
	{
            return k;
	}
