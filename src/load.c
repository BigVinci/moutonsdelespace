...
//
mem load(char* name, char* adress) 
{
    ...
    if (adress != NULL)
        sscanf(adress, "%x", &next_segment_start);
...
/* alloue le segment [heap] qui restera vide */
    ...
    if(next_segment_start>0x4000)
    {
        next_segment_start+= ((memory->seg[j-1].size._32+0x1000)>>12 )<<12; // on arrondit au 1k suppérieur
